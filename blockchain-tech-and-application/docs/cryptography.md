...
4. 可疑交易发生：把钱转走
5. 写错地址：没办法 保存在utxo中
6. op_return返回错误：验证交易合法性时不会被执行
7. 矿工偷答案：发布的区块存在coinbase tx的地址，偷答案会改变merkle tree
8. 交易费：事先不需要说明给谁
...
- 哈希指针通过网络进行传播：系统中用的只是哈希，指针是形象说法。
全节点把值存在数据库里：(key value) level DB
- 区块恋：不合适。不应该用截断私钥的方法，而是多重签名。
- 为什么比特币系统可以绕过分布是共识中的不可能结论：严格意义上没有取得真正共识，随时可能被推翻。
- 量子计算机问题
